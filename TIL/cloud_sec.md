...
- AWS WAF와 AWS Shield
  ...
  - AWS WAF 특징
    - 트래픽을 모니터링하여 공격을 방어해주는 서비스로 AWS의 여러 서비스와 연동
    - 실제 공격 발생 시, 해당 트래픽이 서비스에 도달하지 않도록 하여 보호
    - 여러 조건에 따라 트래픽을 제어 가능
    - AWS WAF 규칙
      - AWS Managed 규칙 : AWS에서 직접 생성한 규칙
      - 유저 생성 규칙 : 조건 등에 따라 Black, Allow , Count, CAPTCHA 직접 구성
    - AWS Shield의 개념
      - AWS에서 실행되는 애플리케이션을 보호하는 DDos 보호 서비스
      - AWS Shield는 애플레킹션 가동 중지 및 지연 시간을 최소화하는 상시 탐지 및 자동 인라인 통합을 제공하므로 DDoS 보호를 위해 AWS Support를 이용할 필요가 있음
    - AWS Shield 특징
      - DDoS 공격을 방어해주는 서비스
      - 모든 AWS 계정에 기본적으로 적용되어 있음
      - 무료 버전과 어드밴스 버전이 있음 

# Lambda 및 이벤트 기반 보안 자동화하기
- 이벤트 기반 아키텍처
  - 이벤트의 특성
...
- AWS Lambda
  - 서버를 프로비저닝 또는 관리하지 않고도 실제로 모든 유형의 애픎리케이션 또는 백엔드 서비스에 대한 코드를 실행할 수 있는 이벤트 중심의 서버리스 컴퓨팅 서비스
  - 특징
    - 다양한 언어 지원 : JAVA, C#, Go, Node, Js, Python, Ruby
    - Lambda의 두 가지 호출 방법 : Event 기반, 직접 혹은 API Gateway를 통해서
