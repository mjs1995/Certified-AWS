...
- 생성형 AI 사용 시 기능 및 과제
  ...
  - 생성형 AI 애플리케이션 수명 주기
    ...
    - 파운데이션 모델 선택
      - 식별된 요구 사항을 기반으로 적절한 생성형 AI 모델을 기존 사전 훈련된 모델에서 선택하거나 처음부터 개발합니다. 
      - 이 결정은 적절한 사전 훈련된 모델의 가용성, 사용 사례의 복잡성, 훈련을 위한 도메인별 데이터의 가용성과 같은 요인에 따라 달라집니다.
    - 성능 개선
      - 선택 또는 개발된 생성형 AI 모델은 애플리케이션의 코드베이스 또는 인프라에 통합됩니다. 
      - 여기에는 모델의 입력 및 출력 형식을 조정하고, 애플리케이션별 데이터로 모델을 파인 튜닝하고, 필요한 사용자 지정 또는 최적화를 구현하는 작업이 포함될 수 있습니다.
    - 결과 평가
      - 철저한 테스트 및 평가를 수행하여 통합된 생성형 AI 기능이 지정된 요구 사항을 충족하고 예상된 성능을 발휘하는지 확인합니다. 
      - 여기에는 다양한 입력, 엣지 케이스 및 실제 시나리오를 사용한 테스트와 생성된 콘텐츠의 품질, 일관성 및 관련성 평가가 포함될 수 있습니다.
    - 배포
      - 테스트에 성공하면 생성형 AI 기능이 통합된 애플리케이션을 프로덕션 환경에 배포합니다. 
      - 생성형 AI 모델의 출력과 관련된 성능, 사용량, 잠재적 문제 또는 편향을 추적하기 위해 모니터링 메커니즘을 구축합니다.
- 사용사례 정의
  - 비즈니스 사용 사례
    - 사용 사례 이름 : 사용 사례를 식별하는 설명이 포함된 짧은 이름
    - 간략한 설명 : 사용 사례의 목적 및 목표에 대한 개략적 요약
    - 행위자 : 시스템 또는 프로세스와 상호 작용하는 주체 또는 이해 관계자, 이는 인간 행위자(예: 고객 또는 직원) 또는 외부 시스템일 수 있습니다. 
    - 사전 조건 : 사용 사례를 시작하기 전에 충족해야 하는 조건
    - 기본 흐름(주요 성공 시나리오) : 사용 사례가 성공적으로 완료되었을 때 발생하는 작업 및 상호 작용에 대한 단계별 설명(처음부터 끝까지), 이는 기본 경로 또는 원활한 경로입니다. 예를 들어 성공을 달성하는 데 필요한 각 단계의 목록입니다.
    - 대체 흐름(확장) : 예외적 조건, 오류 또는 대체 사용자 선택으로 인해 발생할 수 있는 추가 시나리오 또는 경로, 이는 시스템이 이러한 상황을 처리하는 방법(예: 비상 계획)을 설명합니다.
    - 사후 조건 : 사용 사례가 성공적으로 완료된 후 충족되어야 하는 상태 또는 조건
    - 비즈니스 규칙 : 사용 사례의 컨텍스트 내에서 시스템 또는 프로세스의 동작을 제어하는 모든 비즈니스 정책, 제약 조건 또는 규정 
    - 비기능적 요구 사항 : 사용 사례와 관련된 성능, 보안 또는 사용성 고려 사항과 같은 모든 비기능적인 요구 사항
    - 가정 : 사용 사례가 유효하거나 적용 가능하기 위해 필요한 시스템, 환경 또는 컨텍스트에 대한 모든 가정
    - 참고 사항 또는 추가 정보 : 사용 사례를 이해하거나 구현하는 데 도움이 될 수 있는 추가 참고 사항, 설명 또는 보충 정보
  - 생성형 AI를 통한 비즈니스 사용 사례 해결
    - 주요지표
      - 비용 절감
        - 주요 지표 중 하나는 생성형 AI를 사용하여 달성할 수 있는 잠재적 비용 절감입니다. 여기에는 인건비 절감, 프로세스 최적화, 효율성 향상이 포함됩니다.
      - 시간 절약
        - 생성형 AI는 다양한 태스크를 자동화하고 간소화하여 시간을 크게 절약할 수 있습니다. 특정 프로세스 또는 활동에 필요한 시간 단축을 측정하는 것은 중요한 지표가 될 수 있습니다.
      - 품질 개선
        - 생성형 AI는 서면 콘텐츠, 창의적인 디자인, 분석 인사이트와 같은 출력의 품질을 개선할 수 있습니다. 정확성, 일관성, 창의성과 같은 지표를 사용하여 품질 개선을 측정할 수 있습니다.
      - 고객 만족도
        - 생성형 AI를 사용하여 고객 상호 작용 또는 경험을 개선하는 경우 고객 만족도 점수, 순수 고객 추천 지수(NPS), 감정 분석과 같은 지표가 중요한 지표가 될 수 있습니다.
      - 생산성 향상
        - 생성형 AI는 인간의 역량을 강화하여 생산성을 높일 수 있습니다. 산출량, 오류율, 태스크 완료 시간과 같은 지표를 통해 생산성 향상을 측정할 수 있습니다.
    - 접근 방식 
      - 프로세스 자동화
        - 생성형 AI는 콘텐츠 생성, 데이터 분석, 고객 서비스 상호 작용과 같이 반복적이거나 시간이 많이 걸리는 태스크를 자동화하는 데 사용할 수 있습니다. 이러한 접근 방식은 상당한 효율성 향상과 비용 절감으로 이어질 수 있습니다.
      - 의사 결정 증강
        - 생성형 AI는 인사이트, 권장 사항 및 의사 결정 지원을 제공하여 의사 결정 프로세스를 개선하는 데 사용할 수 있습니다. 
        - 생성형 AI 모델은 대규모의 복잡한 데이터세트를 분석함으로써 패턴, 추세, 실행 가능한 인사이트를 발견할 수 있습니다. 
        - 이를 통해 정보에 입각하여 비즈니스 의사 결정을 수립 및 개선하고 궁극적으로 더 나은 성과를 얻을 수 있습니다.
      - 개인화 및 사용자 지정
        - 생성형 AI를 사용하여 고객 또는 이해 관계자를 위한 개인화되고 맞춤화된 콘텐츠, 제품 또는 경험을 생성할 수 있습니다. 
        - 이러한 접근 방식은 고객 만족도, 참여도 및 충성도를 향상할 수 있습니다.
      - 창의적인 콘텐츠 생성
        - 생성형 AI를 사용하여 텍스트, 이미지, 동영상 또는 오디오와 같은 창의적인 콘텐츠를 생성할 수 있습니다. 
        - 이 접근 방식은 마케팅, 광고, 엔터테인먼트 또는 교육 목적으로 유용할 수 있습니다.
      - 탐색적 분석 및 혁신
        - 생성형 AI는 새로운 조합 또는 변형을 생성하여 새로운 아이디어, 개념 또는 솔루션을 탐색하는 데 사용할 수 있습니다. 
        - 이러한 접근 방식은 혁신을 촉진하고 기업이 기술을 선도하는 데 도움이 될 수 있습니다.
- FM 선택
  - 사전 훈련된 모델 선택 기준
    - 비용
      - 사전 훈련된 모델은 특히 복잡한 대규모 모델의 경우 비용이 많이 들 수 있습니다. 
      - 비용에는 라이선스 비용, 추론을 위한 계산 리소스, 잠재적 사용자 지정 또는 파인 튜닝 비용이 포함될 수 있습니다. 
      - 예산 제약을 평가하고 예상 비용 편익을 분석하는 것이 중요합니다.
    - 모달리티
      - 생성형 AI 모델은 텍스트 생성, 이미지 생성, 오디오 생성, 멀티모달 생성(여러 모달리티를 결합)과 같은 다양한 모달리티로 설계될 수 있습니다. 
      - 모달리티 선택은 원하는 출력 형식과 대상 애플리케이션에 따라 달라집니다.
    - 지연 시간
      - 일부 애플리케이션은 실시간 생성 또는 지연 시간이 짧은 생성이 필요하고 다른 애플리케이션은 더 긴 처리 시간을 수용할 수 있습니다. 
      - 대상 사용 사례에 적절한 지연 시간을 보장하려면 모델의 추론 속도와 사용 가능한 계산 리소스를 평가해야 합니다.
    - 다국어 지원
      - 애플리케이션에서 여러 언어로 콘텐츠를 생성해야 하는 경우 원하는 언어를 지원하거나 전이 학습과 같은 기술을 통해 새로운 언어에 적용할 수 있는 모델을 선택하는 것이 중요합니다.
    - 모델 크기
      - 일반적으로 모델이 클수록 계산 요구 사항이 더 높고 추론하는 동안 리소스를 더 많이 사용할 수 있습니다. 
      - 그러나 복잡한 태스크에서 성능이 더 좋은 경우가 많습니다. 모델 크기는 사용 가능한 계산 리소스 및 성능 요구 사항과 균형을 이루어야 합니다.
    - 모델 복잡성
      - 트랜스포머 아키텍처 또는 대규모 언어 모델을 기반으로 하는 모델과 같이 더 복잡한 모델은 고급 태스크를 처리할 수 있지만 배포 및 최적화가 더 어려울 수 있습니다. 
      - 리소스가 제한된 환경 또는 더 단순한 사용 사례에는 더 간단한 모델이 선호될 수 있습니다.
    - 사용자 지정
      - 일부 사전 훈련된 모델은 특정 도메인 또는 태스크에 맞게 파인 튜닝하거나 적응시킬 수 있는 기능을 제공합니다. 
      - 이 사용자 지정은 성능을 향상시킬 수 있지만 추가 계산 리소스와 레이블이 지정된 데이터가 필요할 수 있습니다.
    - 입력/출력 길이
      - 생성형 모델은 처리할 수 있는 최대 입력 또는 출력 시퀀스 길이에 제한이 있을 수 있습니다. 
      - 광범위한 입력 데이터의 긴 형식 생성 또는 처리가 필요한 애플리케이션에서는 원하는 입력/출력 길이를 처리할 수 있는 모델을 고려해야 합니다.
    - 책임 고려 사항
      - 잠재적 편향, 잘못된 정보 위험, 오용과 같이 사전 훈련된 생성형 AI 모델 사용의 책임 영향을 평가하는 것이 중요합니다. 
      - 훈련 데이터 소스와 잠재적인 사회적 영향에 대해 모델을 검증해야 합니다.
    - 배포 및 통합
      - 배포 용이성, 기존 인프라와의 호환성, 모델을 대상 애플리케이션에 통합하기 위한 도구 또는 라이브러리의 가용성을 고려해야 합니다.
  - 선택 기준을 기반으로 사전 훈련된 모델 선택
    - AI21 Labs - Jurassic-2 Series
      - Jurassic-2(J2)는 AI21 Labs의 최첨단 대규모 언어 모델(LLM)입니다. 
      - 기업은 AI21 Jurassic 제품군을 사용하여 기존 조직 데이터를 기반으로 생성형 AI 기반 애플리케이션 및 서비스를 구축합니다. 
      - Jurassic은 긴 형식 및 짧은 형식의 텍스트 생성, 상황에 맞는 질문 답변, 요약, 분류 등 산업 전반의 사용 사례를 지원합니다.
    - Amazon - Titan
      - Amazon Titan 파운데이션 모델은 Amazon Web Services(AWS)에서 대규모 데이터세트로 사전 훈련한 모델 제품군으로, 강력한 범용 모델입니다.
      - 이들 모델을 있는 그대로 사용할 수도 있고, 대용량 데이터를 주석 처리하지 않고 특정 태스크를 위한 자체 데이터로 파인 튜닝하여 실행할 수도 있습니다.
      - Amazon Titan 모델에는 임베딩, 텍스트 생성, 이미지 생성이라는 세 가지 유형이 있습니다.
    - ANTHROP\C - Claude
      - Claude 3은 Anthropic의 최첨단 비전 및 텍스트 AI 모델 제품군입니다. 
      - Haiku, Sonnet, Opus 등 제품군의 세 가지 모델을 통해 고객은 비즈니스 요구에 맞는 인텔리전스, 속도, 비용의 정확한 조합을 선택할 수 있습니다.
    - Cohere - Command XL
      - Cohere는 프롬프트를 기반으로 텍스트 기반 응답을 생성할 수 있는 생성형 LLM인 Command를 제공합니다. 
      - Cohere 모델은 텍스트 생성, 요약, 카피라이팅, 대화, 추출, 질문 답변과 같은 신뢰할 수 있는 비즈니스 애플리케이션을 지원하는 데이터로 훈련됩니다.
    - Meta - Llama 3
      - Llama는 공개적으로 사용 가능한 데이터를 훈련에 사용하는 대규모 언어 모델 제품군입니다. 
      - 이러한 모델은 임의 길이의 입력 시퀀스를 처리하고 가변 길이의 출력 시퀀스를 생성할 수 있는 트랜스포머 아키텍처를 기반으로 합니다. 
      - Llama 모델의 주요 특징 중 하나는 일관성 있고 문맥적 연관성이 높은 텍스트를 생성할 수 있다는 것입니다.
    - Mistral AI - Mistral Large
      - Mistral AI는 높은 과학적 기준을 갖춘 소규모 크리에이티브 팀입니다. 
      - 획기적인 혁신을 통해 효율적이고 유용하며 신뢰할 수 있는 AI 모델을 구축합니다. 
      - Mistral Large는 대규모 추론 기능이 필요하거나 합성 텍스트 생성, 코드 생성, RAG 또는 에이전트와 같이 고도로 전문화된 복잡한 태스크에 적합합니다.
    - Stability AI - Stable Diffusion
      - Stable Diffusion은 업계 최고의 이미지 생성 모델입니다. Stable Diffusion은 텍스트 입력에서 이미지를 생성할 수 있습니다.
- FM 성능 개선
  - 프롬프트 엔지니어링
    - 프롬프트 엔지니어링은 대규모 언어 모델(LLM)의 강점을 이용하는 가장 빠른 방법입니다. 
    - 프롬프트(일련의 질문, 문장 또는 지침)를 통해 LLM과 상호 작용함으로써 달성하려는 출력의 특정 컨텍스트에 따라 LLM 출력 동작을 조정할 수 있습니다.
    - 프롬프트 엔지니어링의 몇 가지 주요 측면
      - 설계: 모델에 원하는 태스크 또는 출력을 효과적으로 전달하는 명확하고 컨텍스트가 풍부한 프롬프트를 작성합니다.
      - 증강: 예제, 데모, 태스크별 지침과 같은 추가 정보 또는 제약 조건을 프롬프트에 통합하여 모델의 생성 프로세스를 안내합니다.
      - 튜닝: 주로 인적 평가 또는 자동화된 지표를 통해 모델의 출력 및 성능을 기반으로 프롬프트를 반복적으로 개선하고 조정합니다.
      - 앙상블: 여러 프롬프트 또는 생성 전략을 결합하여 출력의 전반적인 품질과 견고성을 개선합니다.
      - 마이닝: 프롬프트 검색, 프롬프트 생성 또는 대규모 프롬프트 라이브러리에서 프롬프트 검색과 같은 기술을 통해 효과적인 프롬프트를 탐색하고 식별합니다.
  - 프롬프트 기법
    - Zero-shot 프롬프팅
    - Few-shot 프롬프팅
    ...
    - ReAct 프롬프팅
  - RAG
    - RAG는 검색 시스템의 기능과 생성형 언어 모델의 기능을 결합하여 고품질의 정보 텍스트 출력을 생성하는 자연어 처리(NLP) 기술입니다.
    - 검색 시스템
      - 이 구성 요소는 지식 기반, 웹 페이지 또는 기타 텍스트 소스와 같은 대규모 텍스트 데이터에서 관련 정보를 검색합니다. 
      - 검색 시스템은 정보 검색, 스파스 인덱싱, 고밀도 검색과 같은 기술을 사용하여 주어진 입력 쿼리 또는 컨텍스트에 대해 가장 관련성이 높은 구절 또는 문서를 식별합니다
    - 생성형 언어 모델
      - 이 구성 요소는 GPT-3, BART, T5와 같이 자연어 텍스트를 생성할 수 있는 사전 훈련된 대규모 언어 모델입니다. 
      - 언어 모델은 입력 쿼리 또는 컨텍스트를 검색된 관련 정보와 함께 사용합니다. 이를 통해 검색된 지식을 자체 이해 및 언어 생성 능력과 결합하는 일관되고 유창한 텍스트 출력을 생성합니다.
