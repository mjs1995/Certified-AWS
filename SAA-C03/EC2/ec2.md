...
# EC2 인스턴스 스토리지 
- EBS 볼륨
  - ![image](https://github.com/mjs1995/muse-data-engineer/assets/47103479/3472b583-19b2-4d8f-828a-0e566750b03d)
  - EBS (Elastic Block Store) 볼륨은 인스턴스가 실행 중일 때 연결할 수 있는 네트워크 드라이브입니다.
  - 인스턴스가 종료된 후에도 데이터를 유지할 수 있도록 해줍니다.
  - 한 번에 하나의 인스턴스에만 마운트할 수 있습니다 (CCP 수준에서)
  - 특정 가용 영역에 바인딩됩니다.
  - 비유: "네트워크 USB 스틱"으로 생각할 수 있습니다.
  - 프리티어 : 월별 무료로 사용 가능한 일반용 (SSD) 또는 자석형 EBS 스토리지 30GB.
  - 물리적 드라이브가 아닌 네트워크 드라이브입니다.
    - 인스턴스와 통신하기 위해 네트워크를 사용하므로 약간의 지연이 발생할 수 있습니다.
    - EC2 인스턴스에서 분리하여 다른 인스턴스에 빠르게 연결할 수 있습니다.
  - 특정 가용 영역에 잠긴 상태입니다.
    - 예를 들어 us-east-1a의 EBS 볼륨은 us-east-1b에 연결할 수 없습니다.
    - 볼륨을 이동하려면 먼저 스냅샷을 생성해야 합니다.
  - 사전에 예약된 용량(GB 단위의 크기 및 IOPS)이 있습니다.
    - 예약된 용량에 대해 청구됩니다.
    - 시간이 지남에 따라 드라이브의 용량을 늘릴 수 있습니다.
- EBS Snapshots
  - ![image](https://github.com/mjs1995/muse-data-engineer/assets/47103479/d907799c-2582-4d49-a144-f16c9b482242)
  - 특정 시점의 EBS 볼륨을 백업(스냅샷)합니다.
  - 스냅샷을 만들기 위해 볼륨을 분리할 필요는 없지만, 권장됩니다.
  - 스냅샷은 가용 영역이나 리전 간에 복사할 수 있습니다.
  - 기능
    - ![image](https://github.com/mjs1995/muse-data-engineer/assets/47103479/7f212692-7c44-4219-b183-3ab1089df2bc)
    - EBS 스냅샷 아카이브
      - 스냅샷을 "아카이브 티어"로 이동하여 비용을 75% 절감할 수 있습니다.
      - 아카이브 복원에는 24시간에서 72시간이 소요됩니다.
    - EBS 스냅샷 휴지통(Recycle Bin)
      - 삭제된 스냅샷을 보존하는 규칙을 설정하여 실수로 삭제한 후에 복구할 수 있습니다.
      - 보존 기간을 지정할 수 있으며 (1일부터 1년까지), 삭제된 스냅샷을 보존합니다.
    - 빠른 스냅샷 복원 (FSR)
      - 스냅샷의 완전 초기화를 강제로 실행하여 처음 사용 시 지연이 없도록 합니다. (추가 비용 발생)
- AMI 
  - AMI = Amazon Machine Image
  - AMI는 EC2 인스턴스의 사용자 정의입니다.
    - 사용자는 소프트웨어, 구성, 운영 체제, 모니터링 등을 추가할 수 있습니다.
    - 소프트웨어가 미리 패키지로 제공되므로 부팅 및 구성 시간이 빠릅니다.
  - AMI는 특정 지역을 위해 구축되며 (지역 간에 복사할 수도 있음)
  - 다음에서 EC2 인스턴스를 시작할 수 있습니다.
    - Public AMI: AWS에서 제공하는 AMI
    - 자체 AMI: 사용자가 직접 생성 및 유지 관리하는 AMI
    - AWS Marketplace AMI: 다른 사용자가 만든 (또는 판매하는) AMI
  - AMI 프로세스 (EC2 인스턴스에서)
    - ![image](https://github.com/mjs1995/muse-data-engineer/assets/47103479/6c07b349-13db-44fc-9ab1-cc4f985124bc)
    - EC2 인스턴스를 시작하고 사용자 정의합니다.
    - 데이터 무결성을 위해 인스턴스를 중지합니다.
    - AMI를 빌드합니다 - 이는 EBS 스냅샷도 생성합니다.
    - 다른 AMI에서 인스턴스를 시작합니다.
- EC2 인스턴스 스토어
  - EBS 볼륨은 성능이 좋지만 "제한적인" 네트워크 드라이브입니다.
  - 고성능 하드웨어 디스크가 필요한 경우 EC2 인스턴스 스토어를 사용하세요.
  - 더 나은 I/O 성능을 제공합니다.
  - EC2 인스턴스 스토어는 중지되면 저장된 데이터를 잃게 됩니다 (일시적).
  - 버퍼/캐시/임시 콘텐츠/임시 데이터에 적합합니다.
  - 하드웨어 장애 시 데이터 손실의 위험이 있습니다.
  - 백업과 복제는 사용자의 책임입니다.
- EBS Volume Types
  - EBS 볼륨은 6가지 유형으로 제공됩니다.
    - gp2 / gp3 (SSD): 다양한 워크로드에 대한 가격과 성능을 균형있게 제공하는 일반적인 SSD 볼륨
    - io1 / io2 (SSD): 고성능 SSD 볼륨으로, 미션 크리티컬이자 지연 시간이 낮고 대용량의 워크로드에 적합합니다.
    - st1 (HDD): 주로 액세스되는 대용량 데이터를 처리하는 데 최적화된 저비용 HDD 볼륨
    - sc1 (HDD): 덜 자주 액세스되는 워크로드에 적합한 가장 저렴한 HDD 볼륨
  - EBS 볼륨은 크기 | 처리량 | IOPS (I/O 작업 수)로 특성화됩니다.
  - 부팅 볼륨으로 사용할 수 있는 것은 gp2/gp3 및 io1/io2 뿐입니다.
  - General Purpose SSD
    - 비용 효율적인 저장소, 저지연
    - 시스템 부트 볼륨, 가상 데스크톱, 개발 및 테스트 환경
    - 1 GiB - 16 TiB
    - gp3:
      - 3,000 IOPS의 기준값과 125 MiB/s의 처리량
      - IOPS를 최대 16,000까지, 처리량을 최대 1000 MiB/s까지 독립적으로 증가 가능
    - gp2:
      - 소형 gp2 볼륨은 IOPS를 3,000까지 버스트할 수 있음
      - 볼륨의 크기와 IOPS가 연결되어 있으며, 최대 IOPS는 16,000
      - GB당 3 IOPS, 즉 5,334 GB에서 최대 IOPS에 도달합니다.
  - Provisioned IOPS (PIOPS) SSD
    - 지속적인 IOPS 성능이 필요한 중요한 비즈니스 애플리케이션
    - 또는 16,000 IOPS 이상이 필요한 애플리케이션
    - 데이터베이스 워크로드에 적합 (저장소 성능과 일관성에 민감한 경우)
    - io1/io2 (4 GiB - 16 TiB)
      - 최대 PIOPS: Nitro EC2 인스턴스에 대해 64,000 및 기타 인스턴스에 대해 32,000
      - 저장소 크기와 독립적으로 PIOPS를 증가시킬 수 있음
      - io2는 io1과 동일한 가격으로 더 높은 내구성과 GiB 당 더 많은 IOPS 제공
    - io2 Block Express (4 GiB - 64 TiB)
      - 서브밀리초 레이턴시
      - 최대 PIOPS: 256,000, IOPS:GiB 비율은 1,000:1
    - EBS Multi-attach를 지원
  - Hard Disk Drives (HDD)
    - 부팅 볼륨으로 사용할 수 없습니다.
    - 125 GiB에서 16 TiB까지의 용량을 제공합니다.
    - Throughput Optimized HDD (st1)
      - 대용량 데이터, 데이터 웨어하우스, 로그 처리에 적합합니다.
      - 최대 처리량 500 MiB/s, 최대 IOPS 500
    - Cold HDD (sc1)
      - 드물게 액세스되는 데이터에 사용합니다.
      - 최소 비용이 중요한 시나리오에 적합합니다.
      - 최대 처리량 250 MiB/s, 최대 IOPS 250
  - 32,000 IOPS 이상을 요할 때에는 io1 또는 io2 볼륨의 EC2 Nitro가 필요함 
- EBS 다중 연결 
  - ![image](https://github.com/mjs1995/muse-data-engineer/assets/47103479/938729eb-4596-4bde-bd05-a8a8074d4ebf)
  - 동일한 EBS 볼륨을 동일한 가용 영역 내에서 여러 EC2 인스턴스에 연결할 수 있습니다.
  - 각 인스턴스는 고성능 볼륨에 대한 완전한 읽기 및 쓰기 권한을 갖습니다.
  - 사용 사례
    - 클러스터링된 Linux 애플리케이션 (예: Teradata)에서 더 높은 애플리케이션 가용성 달성
    - 애플리케이션은 동시 쓰기 작업을 관리해야 합니다.
  - 최대 16개의 EC2 인스턴스 동시에 사용 가능
  - XFS, EXT4 등과 같이 클러스터 인식 파일 시스템을 사용해야 합니다.
- EBS 암호화 
  - EBS 볼륨을 암호화하면 다음과 같은 기능을 제공합니다.
    - 볼륨 내부의 데이터가 암호화되어 안전하게 보호됩니다.
    - 인스턴스와 볼륨 간 이동하는 모든 데이터는 암호화됩니다.
    - 모든 스냅샷은 암호화됩니다.
    - 스냅샷에서 생성된 모든 볼륨도 암호화됩니다.
  - 암호화 및 복호화는 투명하게 처리되며 사용자가 직접 수행할 필요가 없습니다.
  - 암호화는 대기 시간에 미치는 영향이 최소화됩니다.
  - EBS 암호화는 KMS (AES-256)에서 키를 활용합니다.
  - 암호화되지 않은 스냅샷을 복사하면 암호화할 수 있습니다.
  - 암호화된 볼륨의 스냅샷은 암호화됩니다.
  - EBS 볼륨의 암호화: 암호화되지 않은 EBS 볼륨을 암호화합니다.
    - 먼저, 해당 볼륨의 EBS 스냅샷을 생성합니다.
    - 생성된 EBS 스냅샷을 암호화합니다. (복사를 사용하여 암호화)
    - 암호화된 EBS 스냅샷으로부터 새로운 EBS 볼륨을 생성합니다. (이 볼륨은 암호화됩니다.)
    - 이제 암호화된 볼륨을 원래의 인스턴스에 연결할 수 있습니다.
- Amazon EFS – Elastic File System
  - ![image](https://github.com/mjs1995/muse-data-engineer/assets/47103479/2944517b-dbe4-4a6e-b03a-dc89c48d5a58)
  - Amazon EFS (Elastic File System)은 많은 EC2 인스턴스에 마운트할 수 있는 관리형 NFS (Network File System)입니다.
  - EFS는 여러 가용 영역의 EC2 인스턴스와 함께 작동합니다.
  - 높은 가용성과 확장성을 갖추고 있지만 비용이 비싸며, 사용량에 따라 비용이 청구됩니다.
  - 사용 사례: 콘텐츠 관리, 웹 서비스, 데이터 공유, WordPress
  - NFSv4.1 프로토콜을 사용합니다.
  - EFS에 대한 액세스를 제어하기 위해 보안 그룹을 사용합니다.
  - Linux 기반 AMI와 호환됩니다. (Windows는 지원하지 않음)
  - KMS를 사용하여 암호화된 상태로 저장됩니다.
  - POSIX 파일 시스템 (Linux과 유사)으로 표준 파일 API를 갖추고 있습니다.
  - 파일 시스템은 자동으로 확장되며, 사용량에 따라 비용이 청구되며, 용량 계획이 필요하지 않습니다.
  - 성능 및 저장 클래스
    - EFS 규모
      - 동시 NFS 클라이언트 수 천 개, 10GB+/s 처리량
      - 자동으로 페타바이트 규모의 네트워크 파일 시스템으로 확장 가능
    - 범용 모드 (EFS 생성 시 설정)
      - 일반용 (기본값) – 지연 시간이 중요한 사용 사례 (웹 서버, CMS 등)
      - 최대 I/O – 높은 지연 시간, 처리량, 고도로 병렬 처리 (빅 데이터, 미디어 처리 등)
    - 처리량 모드
      - Bursting – 1TB = 50MiB/s + 최대 100MiB/s의 버스트 처리량
      - Provisioned – 저장 용량과 관계없이 처리량을 설정할 수 있음, 예: 1TB 저장용량에 대해 1GiB/s
      - Elastic – 작업 부하에 기반하여 처리량을 자동으로 확장 또는 축소
        - 읽기에 대해 최대 3GiB/s, 쓰기에 대해 최대 1GiB/s까지 처리 가능
        - 예측할 수 없는 작업 부하에 사용됨
  - 스토리지 클래스
    - ![image](https://github.com/mjs1995/muse-data-engineer/assets/47103479/553e5c7f-5842-4115-b230-2d93a669f2e0)
    - 저장 계층 (수명 주기 관리 기능 - N일 후 파일 이동)
      - 표준 (Standard): 자주 액세스되는 파일에 사용됨
      - 적은 액세스 (EFS-IA): 파일을 검색하는 데 드는 비용이 낮고 저장 비용이 낮음. 수명 주기 정책으로 EFS-IA 활성화
    - 가용성과 내구성
      - 표준 (Standard): 다중 가용 영역(Multi-AZ), 프로덕션 용이
      - 하나의 영역 (One Zone): 하나의 가용 영역, 개발 및 백업에 적합, IA와 호환됨 (EFS One Zone-IA)
    - 비용 절감률 90% 이상
- EBS vs EFS 
  – Elastic Block Storage
    - ![image](https://github.com/mjs1995/muse-data-engineer/assets/47103479/f13abd7f-677b-4d9a-b951-bdfb3ea38221)
    - EBS 볼륨...
      - 하나의 인스턴스에만 연결 가능 (다중 연결 io1/io2 제외)
      - 가용 영역(AZ) 단위로 잠긴다.
      -  gp2: 디스크 크기가 증가하면 IO도 증가한다.
      -  io1: IO를 독립적으로 증가시킬 수 있다.
    - AZ 간에 EBS 볼륨을 마이그레이션하려면
      - 스냅샷을 생성한다.
      - 스냅샷을 다른 AZ로 복원한다.
      - EBS 백업은 IO를 사용하므로 애플리케이션이 많은 트래픽을 처리 중일 때 백업을 실행하지 않는 것이 좋다.
    - EC2 인스턴스가 종료되면 기본적으로 루트 EBS 볼륨도 종료된다. (해당 기능은 비활성화할 수 있다)
  - Elastic File System
    - ![image](https://github.com/mjs1995/muse-data-engineer/assets/47103479/3ac190e2-1a9d-4f2e-8754-a35e9eb9c646)
    - 다수의 가용 영역(AZ)에 인스턴스를 마운트하기
    - EFS를 사용하여 웹사이트 파일 (WordPress 등) 공유
    - Linux 인스턴스에만 사용 가능 (POSIX 호환)
    - EFS는 EBS에 비해 높은 가격대를 가지고 있다.
    - 비용 절감을 위해 EFS-IA를 활용할 수 있다.
    - 기억해야 할 점: EFS vs EBS vs 인스턴스 스토어
      - EFS는 사용한 만큼만 비용이 청구
      - EBS의 경우에는 실제 사용한 양이 아니라 EBS 드라이브의 크기에 따라 실제 사용량이 아니라 정해진 사용량을 지불하는 식
      - EFS는 다수의 인스턴스에 걸쳐 연결해야 하는 네트워크 파일 시스템에 적합
      - 반면 EBS는 네트워크 볼륨을 한 번에 하나의 인스턴스에 연결할 수 있고 특정 AZ 내로 한정
      - 인스턴스 스토어는 EC2 인스턴스에 IO를 최대로 사용하게끔 해주지만, 인스턴스가 망가지면 함께 망가지는 임시 드라이브
